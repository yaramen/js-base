(function () {
    ...
    const sumElement = document.querySelector('[data-order-sum]')
    ...
    function updateOrder() {
        ...
        const productInCard = orderStore.get()
        const orderIds = Object.keys(productInCard);
        let sum = 0;
        for(const orderId of orderIds) {
            const count = productInCard[orderId];
            const price = products[orderId].price
            sum += count * price;
        }
